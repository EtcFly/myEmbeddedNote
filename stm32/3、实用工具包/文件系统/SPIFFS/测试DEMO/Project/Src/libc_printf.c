...
/**************************************************************************/
// if want to mapping the function to other devices , you need to config those 
//function
/**************************************************************************/
int libc_putchar(int ch)
{
	if (ComSend(COM1, (uint8_t *)&ch, 1) == 0) {
		return ch;
	}
	return SYS_EOF;
}
...
static int rt_strlen(const char *s)
{
    const char *sc;
...
int libc_puts(const char *string)
{
	...
	while ('\0' != *string)
	{
		if (libc_putchar(*string++) == SYS_EOF){
			retV = SYS_EOF;
			break;
		}
	}
...
char *libc_gets(char *str)
{
	...
	int ch;
	for (i=0; i<MAX_PRINTF_BUFFER_SIZE-1; i++) 
	{
		ch = libc_getchar();
		if (SYS_EOF == ch) {
			if (0 == i) {
...
int libc_vsnprintf (char * buf, size_t size, const char * fmt, va_list args)
{
    ...
    for (; *fmt ; ++fmt)
    {
        ...
        else if (qualifier == 'l')
        {
            ...
            if (flags & SIGN) num = (uint32_t)num;
        }
        else if (qualifier == 'h')
        {
...
{
	int retV = SYS_RETURN_ERR;
	if (SYS_NULL == fmt || SYS_NULL == str)	{
		return retV;
	}
	return libc_vsnprintf(str, MAX_PRINTF_BUFFER_SIZE, fmt, args);
}

int libc_sprintf(char *buf, const char *fmt, ...)
{
    int32_t n;
    va_list args;

    va_start(args, fmt);
    n = libc_vsprintf(buf, fmt, args);
    va_end(args);
...
int libc_snprintf(char *buf, size_t size, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    n = libc_vsnprintf(buf, size, fmt, args);
    va_end(args);

    return n;
}

extern void com_sendStr(uint8_t *s);
void libc_printf(const char *fmt, ...)
{
	static char log_buf[MAX_PRINTF_BUFFER_SIZE];
	va_list ap;
	va_start(ap, fmt);
	
	libc_vsprintf(log_buf, fmt, ap);
	libc_puts((const char *)log_buf);
