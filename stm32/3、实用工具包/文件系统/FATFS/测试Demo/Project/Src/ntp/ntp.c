...
uint8_t NTP_Server_IP[4]={203, 107, 6, 88};
...
uint8_t NTP_Retry_Cnt=0; //counting the ntp retry number
    ...
    switch(getSn_SR(SOCK_NTP))
    {
      case SOCK_UDP:
        if ((len = getSn_RX_RSR(SOCK_NTP)) > 0) 		
        {
          ...
          wizchip_recvfrom(SOCK_NTP, data_buf, len, (uint8_t*)&destip, &destport);	
          //printf("%d\r\n", data_buf);
        ...
        */
        ...
        {  
          ...
          {
            wizchip_sendto(SOCK_NTP,NTP_Message, sizeof(NTP_Message), NTP_Server_IP, NTP_Port);
            NTP_Retry_Cnt++;
        ...
        break;
      case SOCK_CLOSED:
        wizchip_socket(SOCK_NTP,Sn_MR_UDP,NTP_Port,0);
        break;
    }
  } 
}
