...
/**
  ...
  * @retval:无
  */ 
void SI4463_DelayUs(uint16_t Us)
{
	uint16_t i;
	while(Us--)
	{
...
/**
  ...
  * @retval:无
  */
void SI446x_CLR_Interrupt_Status( uint8_t rxbyte )
{
    uint8_t l_Cmd[ 4 ] = { 0 };
...
/**
  ...
  * @retval:无
  */
void SI446x_PACKET_Interrupt_Status( uint8_t *pRead )
{
    uint8_t l_Cmd[2] = { 0 };
    ...
    SI446x_Write_Cmds( l_Cmd, 2);		//发送中断读取命令
...
{
    SI_SET_SDN_HIGH( );		//关设备
		SI4463_DelayUs( 300 );		//延时 等待设备完全断电 300US
    SI_SET_SDN_LOW( );		//开设备
		SI4463_DelayUs(5000);  //power on reset     5MS
	  SI_SET_CSN_HIGH( );		//取消SPI片选
}
...
void SI446x_Init( void )
{
	...
	SI446x_CLR_Interrupt_Status(0);//清中断
